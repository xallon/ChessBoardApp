...
namespace ChessBoardApp
{
    public partial class Form1 : Form
    {
        ...
        String[,] board = new String[8, 8] { { "A8", "B8", "C8", "D8", "E8", "F8", "G8", "H8" } , { "A7", "B7", "C7", "D7", "E7", "F7", "G7", "H7" }
        ...
        public Form1()
        {
            ...
            chess[x, y] = 1;
            paintHorse(board[x, y]); // tasi ilk konumuna yerlestiriyor
            // !---------
            print(chess); // tahtanın ilk hali
        }
        ...
        private void Timer1_Tick(object sender, EventArgs e)
        {
            ...
            two_forward = getRandomInteger(0, 4);
            Console.WriteLine(two_forward);
            if (two_forward == 0 || two_forward == 1)
            {
                one_forward = getRandomInteger(2, 4);
                Console.WriteLine(one_forward);
            }
            else
            {
                one_forward = getRandomInteger(0, 2);
                Console.WriteLine(one_forward);
            }
            // !---------
            location[0] = x;
            location[1] = y;
            horse = new Horse(location, two_forward, one_forward);

            if (location[0] <= 7 && location[1] <= 7 && location[1] >= 0 && location[0] >= 0 && chess[location[0], location[1]] != 9)
            {
                chess[x, y] = 9;
                chess[location[0], location[1]] = 1;
                Console.WriteLine(board[location[0], location[1]]);
                paintHorse(board[location[0], location[1]]);
                paint(board[x, y]);
                x = location[0];
                y = location[1];
            }
